...
OUT_DIR = "/root/Documents/physical-ai-learning-journey/projects/omniglam-sdg/outputs_v3"
...
# ---- WIDE RANGE BULLET TIP COLOURS ----
# Covers reds, pinks, berries, mauves, nudes, corals, plums
bullet_colours = [
    (0.55, 0.02, 0.02), (0.75, 0.05, 0.05), (0.35, 0.02, 0.02),  # reds
    (0.80, 0.15, 0.35), (0.64, 0.05, 0.15), (0.90, 0.25, 0.45),  # pinks
    (0.15, 0.00, 0.09), (0.25, 0.02, 0.18), (0.40, 0.05, 0.28),  # berries
    (0.92, 0.41, 0.72), (0.75, 0.35, 0.60), (0.60, 0.25, 0.45),  # mauves
    (0.86, 0.32, 0.35), (0.93, 0.57, 0.59), (0.70, 0.42, 0.44),  # blushes
    (0.78, 0.21, 0.14), (0.88, 0.46, 0.37), (0.65, 0.30, 0.22),  # corals
    (0.55, 0.35, 0.30), (0.70, 0.50, 0.42), (0.80, 0.62, 0.52),  # nudes
]
...
# ---- BUILD 7 LIPSTICK TUBES — all class "lipstick" ----
lipstick_prims = []
for idx, position in enumerate(tube_positions):
    name = f"lipstick_{idx}"
    tube = rep.functional.create.reference(
        usd_path=ASSET_PATH, parent="/World",
        name=name, position=position, rotation=(90,0,0)
    )

    # Single class label — lipstick
    rep.functional.modify.semantics(tube, {"class": "lipstick"}, mode="add")

    # Initial gold case material
    ...
    print(f"Added lipstick_{idx}")

# ---- CAMERA ----
cam = rep.functional.create.camera(
    position=(0,-12,6), look_at=(0,0,2.8),
...
print("Scene ready — OmniGlam SDG v3!")

# ---- CAPTURE LOOP ----
async def run_pipeline():
    num_frames = 1000
    rng = np.random.default_rng(42)
    print(f"Starting OmniGlam SDG v3 — {num_frames} frames...")
    ...
    for i in range(num_frames):
        ...
        # Pick case variant — same for all visible tubes this frame
        case_variant = case_variants[int(rng.integers(0, len(case_variants)))]

        # Tube visibility — always at least 1, max 4
        if i % 8 == 0:
            num_visible = 1
            ...
            num_visible = int(rng.integers(1, 5))
        ...
        for j, prim in enumerate(lipstick_prims):
            ...
            if j in visible_indices:
                ...
                # Randomise bullet tip colour independently per tube
                bullet_colour = bullet_colours[int(rng.integers(0, len(bullet_colours)))]
                bullet_tip = stage.GetPrimAtPath(f"/World/lipstick_{j}/body_top/Cylinder_004")
                if bullet_tip.IsValid():
                    rep.functional.create.material(
                        mdl="OmniPBR.mdl", bind_prims=[bullet_tip],
                        diffuse_color_constant=bullet_colour,
                        metallic_constant=0.0, reflection_roughness_constant=0.4,
                        enable_emission=False
                    )

                # Randomise position and tilt
                base = tube_positions[j]
    ...
    writer.detach()
    rp.destroy()
    print("OmniGlam SDG v3 complete!")
